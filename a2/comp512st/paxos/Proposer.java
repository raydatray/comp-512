...
public class Proposer {
    ...
    public void sendMessage(Ballot ballotToPropose, GameMove move) {
        while (true) {
            ProposerInstance proposerInstance = new ProposerInstance(this.gclReader, this.gclWriter, this.logger, this.majority, ballotToPropose, move);
            ...
            if (installed) {
                ...
                if (proposerInstance.committedOriginalMove()) {
                    ...
                    // forward the move
                    ProposerState state = proposerInstance.getFinalProposerState();

                    // we should tell the proposer state to take the highest ballot + 1
                    ballotToPropose = ballotGenerator.nextTurn(state.ballotToPropose.turn());
                    continue;
                }
            }
