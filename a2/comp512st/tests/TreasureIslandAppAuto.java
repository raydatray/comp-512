...
public class TreasureIslandAppAuto implements Runnable {
    ...
    public TreasureIslandAppAuto(
        ...
        try {
            ...
            if (updDisplay != null && updDisplay.equalsIgnoreCase("true")) {
                ...
                logger.info(
                    "UPDATEDISPLAY is set to " +
                        updateDisplay +
                        ", display will be constantly updated."
                );
            }
        } catch (SecurityException se) {
            logger.log(
                Level.WARNING,
                "Encountered SecurityException while trying to access the environment variable UPDATEDISPLAY.",
                se
            );
        }
        ...
        while (
            ...
            try {
                ...
                if (keepExploring) logger.log(
                    Level.SEVERE,
                    "Encountered InterruptedException while waiting for messages.",
                    ie
                );
                break;
            }
    ...
    public synchronized void move(
        int playerNum,
        char direction,
        boolean displayIsland
    ) {
        ti.move(playerNum, direction);
        if (displayIsland) ti.displayIsland();
    }

    private static class AutoMoveGenerator {

        private static final String[] VALIDMOVES = { "L", "R", "U", "D" };
        public static final Map<String, String> FAILMODECODES = new HashMap<>();
        ...
        public AutoMoveGenerator(
            ...
            if (this.failmode != null && FAILMODECODES.get(failmode) == null) {
                logger.log(
                    Level.SEVERE,
                    "Incorrect failmode parameter " + failmode
                );
            ...
            logger.info(
                "AutoMoveGenerator setup maxmoves = " +
                    this.maxmoves +
                    ", interval = " +
                    this.interval +
                    ", randseed = " +
        ...
        public String nextMove() {
            ...
            if (
                currenttime - lastmovetime < interval // we have some more time left before next move, so pause a bit.
            ) try {
                Thread.sleep(interval - (currenttime - lastmovetime));
            } catch (InterruptedException ie) {
                logger.log(
                    Level.SEVERE,
                    "Encountered InterruptedException while pausing for the next move."
                );
            }

            if (
                totalmoves == maxmoves // we have done max moves.
            ) {
            ...
            if (
                failmode != null && !sendfail && moveid < 5 // For lower numbers, initiate a failure, if enabled.
            ) {
                sendfail = true;
                lastmovetime = clock.millis();
    ...
    public static void main(String args[])
        ...
        System.setProperty(
            ...
            "%1$tl:%1$tM:%1$tS.%1$tN %1$Tp %2$s %4$s: %5$s%6$s%n"
        );
        ...
        try {
            ...
            throw new RuntimeException(
                "SecurityException while initializing process log file."
            );
        } catch (IOException ie) {
            throw new RuntimeException(
                "IOException while initializing process log file."
            );
        }
        ...
        AutoMoveGenerator moveGen = new AutoMoveGenerator(
            maxmoves,
            interval,
            randseed,
            failmode,
        ...
        while (
            true // Just keep polling for the user's input.
        ) {
            ...
            switch (cmd) {
                ...
                default:
                    logger.warning(
                        "Command " +
                            cmd +
                            " is not a valid command for this app."
                    );
            }
        }
